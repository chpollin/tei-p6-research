...
Follow `knowledge/operations.md` § Build assertions for the synthesis procedure and the review prompt, `knowledge/schema.md` § Assertion for frontmatter and section skeleton, and `knowledge/verification.md` for the machine-review protocol, its independence and the counterevidence search. The hard rules in `CLAUDE.md` apply unchanged, in particular that an own conclusion becomes a posit in the output and never an assertion.
...
6. Run machine review over every assertion-statement pair under `knowledge/verification.md`, and rework whatever falls below *fully supports*. Before an assertion supports a design requirement, run and record the counterevidence search defined there.
