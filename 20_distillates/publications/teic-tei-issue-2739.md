...
- In TEIC/TEI issue 2739, the issue author writes that the technical declaration of elements such as `persName` lists the attribute classes. ^s1
...
- In TEIC/TEI issue 2739, the issue author writes that `att.personal`, `att.naming` and `att.canonical` are listed as if they were subclasses of the previous one, which is not the case. ^s2
...
- In TEIC/TEI issue 2739, the issue author asks whether this results from an error in the ODD. ^s3
  > "Maybe this results from an error in the ODD?" (https://github.com/TEIC/TEI/issues/2739, issue body, 2025-08-04)
- In TEIC/TEI issue 2739, a commenter states that `att.personal` is a member of `att.naming` and that `att.naming` is a member of `att.canonical`. ^s4
...
- In TEIC/TEI issue 2739, a commenter writes that it can be seen from their spec files. ^s5
  > "as can be seen from their spec files" (https://github.com/TEIC/TEI/issues/2739, first comment, 2025-08-05)
- In TEIC/TEI issue 2739, a commenter gives the URL of the `att.personal` specification file on the repository's `dev` branch. ^s6
...
- In TEIC/TEI issue 2739, a commenter asks whether, if that were true, the hierarchy would have to be reverse. ^s7
...
- In TEIC/TEI issue 2739, a commenter writes that right now the display suggests that `att.naming` is a member of `att.personal`. ^s8
...
- In TEIC/TEI issue 2739, a commenter asks whether `att.naming` should really be called `att.personal.naming` and `att.canonical` `att.personal.naming.canonical`. ^s9
...
- In TEIC/TEI issue 2739, a commenter writes that attribute classes do not behave in quite the same way as model classes. ^s10
...
- In TEIC/TEI issue 2739, a commenter writes that in neither case is the naming as systematic as proposed. ^s11
...
- In TEIC/TEI issue 2739, a commenter writes that it would be a fairly major upheaval to make all class names reflect the hierarchy. ^s12
...
- In TEIC/TEI issue 2739, a commenter suspects that there are some cases where a class inherits from more than one other class. ^s13
...
- In TEIC/TEI issue 2739, a commenter writes that it is not a simple hierarchy anyway. ^s14
  > "so it's not a simple hierarchy anyway" (https://github.com/TEIC/TEI/issues/2739, third comment, 2025-08-06)
- In TEIC/TEI issue 2739, a commenter writes that they obviously still have problems understanding the attribute class hierarchy correctly. ^s15
...
- In TEIC/TEI issue 2739, a commenter writes that they do not want to overdo the harmonisation of the class names. ^s16
