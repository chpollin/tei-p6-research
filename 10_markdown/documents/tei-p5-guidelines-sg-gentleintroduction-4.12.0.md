...
source: '[[00_sources/tei-p5-guidelines-sg-gentleintroduction-4.12.0.xml]]'
converter: tools.ingest_guidelines v1; complete XML plus verbatim blocks of prose,
  specifications and support files
channel: collection
metadata:
  title: TEI P5 4.12.0 A Gentle Introduction to XML
  creator: TEI Consortium
  date: '2026-07-28'
  format: application/xml
  identifier: https://github.com/TEIC/TEI/blob/113e933e21f016e2655518321e9d10214b8d9fcb/P5/Source/Guidelines/en/SG-GentleIntroduction.xml
  license: CC-BY-3.0
  confidential: false
created: '2026-09-07'
updated: '2026-09-07'
...
Source byte length: 90261. Git blob: `2c349634a968ff16210cf769385ab6f3aa863505`.
...
some recommended online courses include <ptr target="http://www.w3schools.com/xml/default.asp"/> and <ptr target="https://www.ibm.com/developerworks/xml/tutorials/xmlintro/xmlintro.html"/>.</note></p>
...
<list rend="bulleted"><item>XML is <hi>extensible</hi>: it does not consist of a fixed set of
tags;</item>
<item>XML documents must be <hi>well-formed</hi> according to a defined
syntax;</item>
<item>an XML document can be formally <hi>validated</hi> against a set of schema rules for consistent application;</item>
...
<div type="div3" xml:id="SG112"><head>Types of Document</head> 
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SG131-egXML-av">... Rosalind's 
remarks <quote>This is the silliest stuff that ere I heard 
of!</quote> clearly indicate ...</egXML>
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SG132-egXML-mc" xml:lang="en" valid="false" source="#SG132-neg-1">
<anthology xmlns="">
  <poem>
...
this: <eg xml:space="preserve"><![CDATA[ <my:line>This is one of my lines</my:line> 
<!-- ... --> 
...
<div type="div2" xml:id="SG14"><head>Validating a Document's Structure</head>
...
text being encoded.</p>
  <p>Schema validation for XML is usually written in the RELAX NG language (<ptr target="http://relaxng.org/"/>) originally
...
with a letter and may not contain a space.<note place="bottom">In XML, a single colon may also
...
been either single patterns or <code>text</code>. We often need to define content models in more complicated ways, in which the components
...
moment), <!--ebb: This note now seems unnecessary, and the linked example isn't about a poem not starting on a line boundary: <note place="bottom">It will not have escaped the astute reader
...
<gi scheme="imaginary">firstLine</gi> and <gi scheme="imaginary">secondLine</gi>; these will have exactly
...
include all three possibilities: <eg xml:space="preserve"><![CDATA[ poem_p = element poem
...
difference between this declaration and the following: <eg xml:space="preserve"><![CDATA[poem_p = element poem
...
<gi scheme="imaginary">line</gi> to <eg xml:space="preserve"><![CDATA[line_p = element
line { (text | name_p )* }]]></eg>
</p>
...
<p>Quite complex models can be built in this way, to match
...
<eg xml:space="preserve"><![CDATA[refrain_p = element refrain {line_p+}
...
<eg xml:space="preserve"><![CDATA[poem_p =
...
<!-- not yet explained: 
  ...
  other things?
-->
<!--2019-12-29 ebb: It's probably not necessary to give a complete introduction to Relax NG here, but we should now point in the main text (not in a footnote) to complete tutorial resources, and give the reader a glimpse of how the TEI schema is made, and how the TEI community customizes it. -->
  <p>It is beyond the scope of this "Gentle Introduction to XML" to provide a complete orientation to schema writing with Relax NG, but interested readers may wish to consult more thorough tutorials on the subject.<note place="bottom">For a complete tutorial introduction to RELAX NG, see <ptr target="#SG-BIBL-1"/>.</note> The examples and explanation provided here may be helpful to consult when reading the schema declarations posted for groupings of TEI elements that share the same content model, such as <ident type="macro">macro.phraseSeq</ident>, whose declaration features an example of mixed text and element content. Due to the complexity of the TEI schema as expressed in Relax NG, it is best to customize its content model in TEI itself by writing an ODD, as discussed in <ptr target="#IM"/>.</p>
</div>
</div>
</div>
...
stanzas and the second a single stanza,  by a tree structure like the following figure:</p>
  <figure xml:id="SG152-figure-jf"><graphic width="80%" url="Images/xmlFlowChart.png" mimeType="image/png"/></figure>
<!--2019-12-28 (ebb): Temporarily (I hope) replacing xmlFlowChart.svg with a png made from a screen shot, until we can support SVG images in epub3 build tests. -->
<!-- 2019-12-26 (ebb): Replaced the fuzzy png graphic that was here with an SVG made from XSLT (Images/xml-to-svg-flowchart.xsl) over a source XML (Images/poems.xml) that mirrors the example here.  -->
 <!-- 2019-12-14 Revised graphic to better correspond with example and XML hierarchy (JL) -->
<p>This graphic represents the hierarchical structure of an XML document,
...
and elsewhere on the Web: good beginners' tutorials include <ptr target="http://dh.obdurodon.org/introduction-xpath.xhtml"/>, <ptr target="http://www.w3schools.com/xml/xpath_intro.asp"/> and <ptr target="http://www.zvon.org/xxl/XPathTutorial/"/>, the latter being
...
<code><![CDATA[/anthology/poem[1]/stanza[2]/line[4]]]></code>. The square brackets
...
the expression <code>/anthology/poem/heading</code> refers to all their headings. </p>
  <p>The forward slash (‘/’, U+002F SOLIDUS) within an XPath expression behaves in much the same way
...
in the XPath expression <code>/anthology/poem</code>, the single slash
...
forward slashes together. For example, the XPath expression <code>/anthology/poem//line</code>
...
in chapter <ptr target="#SA"/> of these Guidelines. These mechanisms all
...
predefined character entities <code><![CDATA[&apos;]]></code> or <code><![CDATA[&quot;]]></code></note>.</p>
<p>For example: 
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SG16-egXML-pf">
  <poem xmlns="" xml:id="Poem1" status="draft"> ... </poem>
</egXML>
...
example, represented here by ellipsis points, the <att>xml:id</att>
...
such as <eg xml:space="preserve"><![CDATA[att.status =
...
<ref target="#XSD2">Datatype Library</ref>,
...
<div type="div3" xml:id="SG-id"><head>Identifiers and Indicators</head>
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SG-id-egXML-gy" xml:lang="en" valid="false">
<poem xmlns="" xml:id="Rose"> ... </poem>

<poem xmlns="" xml:id="P40"> ... </poem>
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SG-id-egXML-wi" xml:lang="en" valid="false">
Blake's poem on the sick rose 
<poemRef xmlns="" target="#Rose"/> ...</egXML>
<!--
...
<eg xml:space="preserve"><![CDATA[graphic_p = element graphic {att.url, empty} att.url =
...
like this: <egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SG-id-egXML-qv" xml:lang="en" valid="false" source="#UND"><poem xmlns=""><graphic url="http://en.wikisource.org/wiki/Image:Blake_sick_rose.jpg"/>
</poem></egXML> By providing a location from which a reproduction of
...
use an XPath such as <code><![CDATA[/anthology/poem[@status='draft']]]></code>. To
find the headings of all such poems, we would use the XPath
<code><![CDATA[/anthology/poem[@status='draft']/heading]]></code>. </p>
</div>
</div>
...
taken from some other <term>namespace</term>. And an XML
...
such flags are called <term>processing instructions</term>. We discuss each of these
three cases in the rest of this section.</p>

<p>The <term>XML declaration</term> should be supplied at the very
...
very beginning of an XML file. For example: <eg><![CDATA[<?xml
version="1.0" encoding="UTF-8"?>]]></eg> The XML declaration specifies
...
character reference (<ptr target="#SG-er"/>). The XML declaration is
...
process the associated text correctly.</p>
...
character reference <code><![CDATA[&#x00E9;]]></code> (the <code>x</code> indicating
that what follows is a hexadecimal value) or
<code><![CDATA[&#0233;]]></code> (its decimal equivalent). References of this type do not need
...
<q>T&amp;C</q> might be encoded as <code><![CDATA[T&amp;C]]></code>. </p>
...
then references such as <code><![CDATA[&TEI;]]></code> may be used within it,
...
<p>A valid XML document necessarily specifies or implies some schema
within which its constituent elements are defined and rules about
their usage are given: this is a part of what it means to be
<q>valid</q>. By contrast, a well formed XML document is not required
to specify a schema at all. Nonetheless, it may well be useful to
indicate that the element names used in it have some defined
provenance. It might also be desirable to include in a document
elements that are defined (possibly differently) in different
schemas. A cabinet-maker's schema might well define an element called
<gi scheme="imaginary">table</gi> with very different characteristics
from those of a documentalist's.</p>
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SGname-egXML-zm" xml:lang="en" valid="false" source="#UND">
<anthology xmlns="">
...
namespace name:
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SGname-egXML-bg" xml:lang="en" valid="false" source="#UND">
<anthology xmlns="">
<!-- anthology markup elements here -->
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SGname-egXML-ni" xml:lang="en" valid="false" source="#UND">
<anthology xmlns="" xmlns:svg="http://www.w3.org/2000/svg">
  <!-- anthology markup elements here -->
   <svg:svg>
      <!-- SVG markup elements here -->
   </svg:svg>
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="SGname-egXML-hn" xml:lang="en" valid="false">
<anthology xmlns="" xmlns:gram="http://www.gram.org" xmlns:svg="http://www.w3.org/2000/svg">
    <!-- anthology markup elements here -->
   <svg:svg>
      <!-- SVG markup elements here -->
   </svg:svg>  
...
<code><![CDATA[&amp;]]></code>, <code><![CDATA[&lt;]]></code>, and
<code><![CDATA[&gt;]]></code> provide one method of dealing with this problem,
...
of the markup.</p>
<p>Here is an example XML processing instruction: <eg><![CDATA[<?tex
\newpage ?>]]></eg> It begins with <code><![CDATA[<?]]></code> and ends with
<code><![CDATA[?>]]></code>. In between are two strings separated by a space:
the first is the name of some processor (<code>tex</code> in the above
...
must be a valid XML name other than <code>XML</code>,
<code>Xml</code>, or <code>xml</code>, etc. The second string can be
any arbitrary sequence of characters, not including the closing
character-sequence <code><![CDATA[?>]]></code>.</p>
</div>
</div>
...
<eg xml:space="preserve"><![CDATA[<!DOCTYPE anthology [
<!ENTITY mdash "&#2014;">
...
<code><![CDATA[&legalese;]]></code> appears, it will be replaced by the
...
<p>In the past, different schema languages adopted entirely different attitudes to
...
The <att scheme="HTML">href</att> <term>pseudo-attribute</term> points to the location of the schema. This is the only mandatory
  pseudo-attribute, but others can be added to give more information about the schema:
  
  <eg xml:space="preserve"><![CDATA[<?xml-model href="burgess.rng" 
...
how to use them.</p>
  ...
  <p>A document instance may be valid according to many
different schemas, each appropriate to a different processing task. All of these may be expressed in the same way:

  <eg xml:space="preserve"><![CDATA[<?xml-model href="tei_tite.xsd" type="application/xml" ?>
<?xml-model href="checkNames.sch" type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron" ?>]]></eg>

This example includes a standard schema in XML Schema format, along with a Schematron schema which might be 
used for checking the format and linking of names.</p>

<p>Any modern XML processing software tool will provide
...
```xml
<head>What's Special about XML?</head>
```
...
of analysis is crucial: the extent to which syntactic structure
and narrative structure mesh, or fail to mesh, for example, or the
extent to which phonological structures reflect morphology.</p>
```
...
nothing between the start and the end-tag; in this case the two may
optionally be merged together into a single composite tag with the
solidus at the end, like this: <tag>quote/</tag>. </p>
```
...
```xml
<head>Content Models: an Example</head>
```
...
```xml
<head>Validating a Document's Structure</head>
```
...
distinct: an element with the GI <gi scheme="imaginary">foo</gi> is
<emph>not</emph> the same as an element with the GI <gi scheme="imaginary">Foo</gi>; the root element of a TEI-conformant
document is <gi>TEI</gi>, <emph>not</emph> <gi scheme="imaginary">tei</gi>.</p>
```
...
XML location: `/div[1]/div[4]/div[1]/div[2]/head[1]`.
...
XML location: `/div[1]/div[4]/div[1]/div[2]/p[1]`.
...
<gi scheme="imaginary">heading</gi>) will lead eventually to <code>text</code>. In our example,
<gi scheme="imaginary">heading</gi> and <gi scheme="imaginary">line</gi> are so defined, since their content
models say <code>text</code> only and name no embedded elements.</p>
```
...
XML location: `/div[1]/div[4]/div[1]/div[3]/head[1]`.
...
XML location: `/div[1]/div[4]/div[1]/div[3]/p[1]`.
...
XML location: `/div[1]/div[4]/div[1]/div[4]/head[1]`.

```xml
<head>Connectors</head>
```
...
XML location: `/div[1]/div[4]/div[1]/div[4]/p[1]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/head[1]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/p[1]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/p[2]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/p[3]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/p[4]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/p[5]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/p[6]`.
...
XML location: `/div[1]/div[4]/div[1]/div[5]/p[7]`.
...
```xml
<head>Complicating the Issue</head>
```
...
XML location: `/div[1]/div[5]/figure[1]`.
...
```xml
<head>Declaring Attributes</head>
```
...
```xml
<head>Identifiers and Indicators</head>
```
...
```xml
<head>Other Components of an XML Document</head>
```
...
XML location: `/div[1]/div[7]/div[1]/head[1]`.

```xml
<head>Character References</head>
```
...
XML location: `/div[1]/div[7]/div[1]/p[1]`.
...
XML location: `/div[1]/div[7]/div[1]/p[2]`.
...
XML location: `/div[1]/div[7]/div[1]/p[3]`.
...
XML location: `/div[1]/div[7]/div[1]/p[4]`.
...
XML location: `/div[1]/div[7]/div[1]/p[5]`.
...
XML location: `/div[1]/div[7]/div[1]/p[6]`.
...
XML location: `/div[1]/div[7]/div[2]/head[1]`.
...
XML location: `/div[1]/div[7]/div[2]/p[1]`.
...
XML location: `/div[1]/div[7]/div[2]/p[2]`.
...
XML location: `/div[1]/div[7]/div[2]/p[3]`.
...
XML location: `/div[1]/div[7]/div[2]/p[4]`.
...
XML location: `/div[1]/div[7]/div[2]/p[5]`.
...
XML location: `/div[1]/div[7]/div[2]/p[6]`.
...
XML location: `/div[1]/div[7]/div[2]/p[7]`.
...
XML location: `/div[1]/div[7]/div[2]/div[1]/head[1]`.

```xml
<head>Marked Sections</head>
```
...
XML location: `/div[1]/div[7]/div[2]/div[1]/p[1]`.
...
XML location: `/div[1]/div[7]/div[2]/div[1]/p[2]`.
...
XML location: `/div[1]/div[7]/div[3]/head[1]`.
...
XML location: `/div[1]/div[7]/div[3]/p[1]`.
...
XML location: `/div[1]/div[7]/div[3]/p[2]`.
...
XML location: `/div[1]/div[8]/div[1]/head[1]`.

```xml
<head>Associating Entity Definitions with a Document
Instance</head>
```
...
XML location: `/div[1]/div[8]/div[1]/p[1]`.
...
XML location: `/div[1]/div[8]/div[1]/p[2]`.
...
XML location: `/div[1]/div[8]/div[1]/p[3]`.
...
DOCTYPE in this example is, of course, the name of the root element of
the document to which this declaration is prefixed; however, only an XML
DTD processor will take note of this fact. </p>
```
...
XML location: `/div[1]/div[8]/div[2]/head[1]`.

```xml
<head>Associating a Document Instance with Its Schema</head>
```
...
XML location: `/div[1]/div[8]/div[2]/p[1]`.
...
XML location: `/div[1]/div[8]/div[2]/p[2]`.
...
XML location: `/div[1]/div[8]/div[2]/p[3]`.
...
XML location: `/div[1]/div[8]/div[3]/head[1]`.

```xml
<head>Assembling Multiple Resources into a Single Document</head>
```
...
XML location: `/div[1]/div[8]/div[3]/p[1]`.
...
XML location: `/div[1]/div[8]/div[3]/p[2]`.
...
XML location: `/div[1]/div[8]/div[4]/head[1]`.

```xml
<head>Stylesheet Association and Processing</head>
```
...
XML location: `/div[1]/div[8]/div[4]/p[1]`.
...
XML location: `/div[1]/div[8]/div[4]/p[2]`.
...
XML location: `/div[1]/div[8]/div[4]/p[3]`.
...
XML location: `/div[1]/div[8]/div[4]/p[4]`.
...
XML location: `/div[1]/div[8]/div[4]/p[5]`.
...
XML location: `/div[1]/div[8]/div[4]/div[1]/head[1]`.

```xml
<head>Content Validation</head>
```
...
XML location: `/div[1]/div[8]/div[4]/div[1]/p[1]`.
...
XML location: `/div[1]/div[8]/div[4]/div[1]/p[2]`.
...
XML location: `/div[1]/div[8]/div[4]/div[1]/p[3]`.
