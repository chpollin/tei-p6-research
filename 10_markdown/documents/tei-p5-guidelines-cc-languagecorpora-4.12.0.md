...
source: '[[00_sources/tei-p5-guidelines-cc-languagecorpora-4.12.0.xml]]'
converter: tools.ingest_guidelines v1; complete XML plus verbatim blocks of prose,
  specifications and support files
channel: collection
metadata:
  title: TEI P5 4.12.0 Language Corpora
  creator: TEI Consortium
  date: '2026-07-28'
  format: application/xml
  identifier: https://github.com/TEIC/TEI/blob/113e933e21f016e2655518321e9d10214b8d9fcb/P5/Source/Guidelines/en/CC-LanguageCorpora.xml
  license: CC-BY-3.0
  confidential: false
created: '2026-09-07'
updated: '2026-09-07'
...
Source byte length: 52663. Git blob: `6b8be10742d4b7b1fa916238ede0389df88282cb`.
...
conscious set of design criteria.</p>
...
as single but composite texts.</p>
...
elements from different TEI modules.</p>
...
importance for corpus creators.</p>
...
<specList><specDesc key="teiCorpus"/><specDesc key="TEI"/><specDesc key="teiHeader"/><specDesc key="text"/><specDesc key="group"/></specList> Full descriptions of these may be found in
...
varieties of composite text in different ways.</p>
...
The <gi>TEI</gi> element may be used in the same manner itself; the
<gi>teiCorpus</gi> element, however, makes explicit the multiplicity
of the collection, whatever it may be. The individual samples in the
corpus are encoded as separate <gi>TEI</gi> elements, and the entire
corpus is enclosed in a <gi>TEI</gi> or <gi>teiCorpus</gi> element.
Each sample has the usual structure for a <gi>TEI</gi> document,
comprising a <gi>teiHeader</gi> followed by one or more members of the
<ident type="class">model.resource</ident> class. The corpus, too,
has a corpus-level <gi>teiHeader</gi> element, in which the corpus as
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCDEF-egXML-fp" valid="feasible" source="#UND"><teiCorpus>
  <teiHeader> <!-- ... --> </teiHeader>
  <TEI>
    <teiHeader> <!-- ... --> </teiHeader>
    <text> <!-- ... --> </text>
...
Or, alternatively:
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCDEF-egXML-zf" valid="feasible" source="#UND"><TEI>
  <teiHeader> <!-- ... --> </teiHeader>
  <TEI>
    <teiHeader> <!-- ... --> </teiHeader>
    <text> <!-- ... --> </text>
...
for date, region, etc. A <gi>teiCorpus</gi> element may occur
directly inside a <gi>teiCorpus</gi> specifically to allow
direct support for reflecting such internal corpus structure in the
markup. For example:
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCDEF-egXML-tw" valid="feasible" source="#NONE"><teiCorpus>
  <teiHeader> <!-- metadata for entire corpus --> </teiHeader>
  <teiCorpus>
    <teiHeader> <!-- metadata for news sub-corpus --> </teiHeader>
    <TEI>
      <teiHeader> <!-- metadata for article #1 --> </teiHeader>
      <text> <!-- transcription of article #1 --> </text>
    </TEI>
    <TEI>
      <teiHeader> <!-- metadata for article #2 --> </teiHeader>
      <text> <!-- transcription of article #2 --> </text>
    </TEI>
    <TEI>
      <teiHeader> <!-- metadata for article #3 --> </teiHeader>
      <text> <!-- transcription of article #3 --> </text>
    </TEI>
    <TEI>
      <teiHeader> <!-- metadata for article #4 --> </teiHeader>
      <text> <!-- transcription of article #4 --> </text>
    </TEI>
  </teiCorpus>
  <teiCorpus>
    <teiHeader> <!-- metadata for editorial sub-corpus --> </teiHeader>
    <TEI>
      <teiHeader> <!-- metadata for editorial #1 --> </teiHeader>
      <text> <!-- transcription of editorial #1 --> </text>
    </TEI>
    <TEI>
      <teiHeader> <!-- metadata for editorial #2 --> </teiHeader>
      <text> <!-- transcription of editorial #2 --> </text>
    </TEI>
  </teiCorpus>
  <teiCorpus>
    <teiHeader> <!-- metadata for reviews --> </teiHeader>
    <TEI>
      <teiHeader> <!-- metadata for review #1 --> </teiHeader>
      <text> <!-- transcription of review #1 --> </text>
    </TEI>
    <TEI>
      <teiHeader> <!-- metadata for review #2 --> </teiHeader>
      <text> <!-- transcription of review #2 --> </text>
    </TEI>
  </teiCorpus>
</teiCorpus>
</egXML></p>
<p>Furthermore, useful groupings
...
characterized for different applications.</p>
...
individual objects for study.</p>
...
largely of other texts.</p>
<p>The <gi>group</gi> element is provided to simplify the encoding of
collections, anthologies, and cyclic works; the <gi>group</gi> element
may also be used to record the potentially complex internal structure
of language corpora. (For a full description, see chapter <ptr target="#DS"/>.) The choice between using <gi>group</gi> or nested
<gi>teiCorpus</gi> elements is up to individual encoders, but in
...
the <gi>text</gi> element. For further discussion, see chapter <ptr target="#DS"/>.</p>
...
process is described in more detail in section <ptr target="#STMA"/>.</p></div>
...
chapters <ptr target="#FS"/> and <ptr target="#FD"/>.</p>
...
such definitions using the mechanisms described in sections <ptr target="#HD43"/>.</p>

<p>Using these situational parameters, an informal domestic
conversation might be characterized as follows:
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAHTD-egXML-di"><textDesc n="Informal domestic conversation">
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAHTD-egXML-ne"><textDesc n="novel">
...
described in <ptr target="#ND"/>.</p>
<p>For example, a participant in a recorded conversation might be
described informally as follows:
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAHPA-egXML-bs"><particDesc xml:id="p2">
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAHPA-egXML-dx"><person sex="2" age="mid">
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAHPA-egXML-ei"><particDesc>
...
structure which use of the <gi>listPerson</gi> element permits.</p>
...
<ident type="module">namesdates</ident> module is also included in the schema.</p>
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAHSE-egXML-in"><settingDesc>
...
way:
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAHSE-egXML-ro"><settingDesc>
   <setting who="#p1 #p2">
...
will be more than one instance of a header element of the relevant type.</p>
...
the corpus or a text header.</item></list></p>
...
element is over-ridden for that text alone.</p>
...
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAS1-egXML-jh" valid="feasible" source="#UND"><TEI>
  <teiHeader>
...
are two, one of them (in this case <val>CorPol1</val>) should be
specified as the default: <egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAS2-egXML-tn"><editorialDecl>
...
structure of a text in which a division states otherwise: <egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAS2-egXML-zj" source="#UND"><text>
  <body>
    <div1 n="D1">  </div1>
...
division D2 will use correction policy <val>CorPol2</val>.</p>
...
for simplicity and ease of processing.</p>
...
identifiers must be of a different kind.</item></list></p>
<p>To demonstrate how these rules operate, we now expand our earlier
example slightly:
<egXML xmlns="http://www.tei-c.org/ns/Examples" xml:id="CCAS2-egXML-ra">
<encodingDesc>
...
that identifies two correction elements (C1A and C2A).</p></div>
...
is discussed in chapter <ptr target="#HD"/>, and in section <ptr target="#CCAH"/> of the present chapter.</p>
...
semantic field, or any combination drawn from these and other domains.</p>
...
annotation methods, the <att>decls</att> attribute should be used to
indicate the fact, as further discussed in section <ptr target="#CCAS"/>.</p>
...
behind this universal approach is further discussed in chapter <ptr target="#AB"/>.
...
elements, or silently omitted, as appropriate.</item></list></p> 
</div>
<div>
  <head>Module for Language Corpora</head>
  <p>The module described in this chapter makes available the
  following components:
  <moduleSpec xml:id="DCCHDR" ident="corpus">
    <idno type="FPI">Metadata for Language Corpora</idno>
    <desc xml:lang="en" versionDate="2018-07-12">Corpus texts</desc>
    <desc xml:lang="fr" versionDate="2018-07-09">Corpus linguistiques</desc>
    <desc xml:lang="zh-TW" versionDate="2018-07-09">文集文本</desc>
    <desc xml:lang="it" versionDate="2018-07-09">Corpus di testi</desc>
    <desc xml:lang="pt" versionDate="2018-07-09">Textos do corpora</desc>
    <desc xml:lang="ja" versionDate="2018-07-09">コーパスモジュール</desc>
  </moduleSpec>
  <!--publicID: -//TEI P5//ELEMENTS Additional Element Set for Language Corpora//EN-->
...
```xml
<head>Varieties of Composite Text</head>
```
...
then no problem arises. If however they require
different modules, then these must be included in the TEI customization. This
process is described in more detail in section <ptr target="#STMA"/>.</p>
```
...
```xml
<head>Contextual Information</head>
```
...
```xml
<head>The Text Description</head>
```
...
```xml
<head>The Participant Description</head>
```
...
this context <val>#ED2</val> is synonymous with the defaults for that
editorial declaration, namely <val>#C2A #N2B</val>, resulting in a list
that identifies two correction elements (C1A and C2A).</p>
```
...
```xml
<head>Linguistic Annotation of Corpora</head>
```
