...
Every fixture page is synthetic markup written for this test in the layout the
LISTSERV archive uses. Live pages carry third-party names and addresses and are
never copied into the repository, so the sender in these fixtures is a
placeholder whose absence from the normalized stream is what the rights test
actually checks.
"""
