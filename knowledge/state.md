...
| 6. Entity run and topic cycles | in progress | the first selection-driven vertical cycle ran on the topic Metadata and Entities (commit `0ac8964`): nine sources, nine validated distillates, thirty-five reviewed assertions, chapter 08 grounded; the second run of the topic admitted twelve sources selected by the recorded procedure (commit `02e2df7`), and all twelve distillates are validated after a two-round review; its assertions and the chapter update follow; the next topics follow the order of the posits of chapter 12 |
