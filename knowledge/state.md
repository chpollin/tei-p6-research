...
| Grounded knowledge | four bounded chapters; entity run reviewed | fourteen document sources and four citation-only sources have distillates; the nine entity distillates of the first run and the twelve of the second run (2026-09-06) are `validated` after fresh-context review under a different model (101 and 141 source pairs, all passing after reformulation rounds); seventy-seven assertions carry a passing machine review, of which seventy-one are `validated` and six stay `contested` as recorded disagreements, four between the Guidelines chapter and the class specifications and two between the announced soft deprecation of `key` in issue 337 and the attribute retained at the pinned release; five counterevidence searches of the second run are recorded in the entity topic map; chapter 08 cites sixty-six assertions with eleven posits after the second run; the four chapters are `grounded`; human verification remains open |
...
| Integrated release gate | passed on 2026-09-06 | 1,362 tests pass and ruff reports no finding. Full-vault validation reports no errors or warnings. The source admissions of both entity runs, the recorded support-review hashes of both runs and of wave one, 68 model cases of version 0.1 and 58 of version 0.2 reproduce; all five pages reproduce byte-for-byte from their recorded build date; the quotation check of the second run passes against the local raw snapshots. The 2026-09-05 browser checks were not repeated. |
| Knowledge navigation | implemented | The browser inventories the current vault, at the last build twenty-three source representations, thirty distillates, seventy-seven assertions and four grounded chapters. Exact passage links and backreferences follow the immediate-layer chain. Citation-only admissions end at their checked quotation and citation. |
