...
phenomena: []                # optional; wikilinks to the glossary entries this assertion is about
related: []                  # optional; wikilinks to any vault document, navigation only
created: 2026-01-01
updated: 2026-01-01
---
```
...
`phenomena` names the glossary entries the assertion speaks about, and `related`
names any document worth reaching from here. Both fields navigate. Neither
carries evidence, so neither may appear in `grounding`; validation raises
`E-PHENOMENON` where a `phenomena` value names a document outside the glossary
and `E-ANCHOR` where either target does not resolve. The generated example list
of a glossary entry is built from `phenomena`, which is what makes the
registration reciprocal without a second hand-kept list. Changing `topics`,
`phenomena` or `related` alone is a change of navigation metadata and requires
no new check date, because `updated` stays as it is and the recorded checks
still cover the content they judged.
...
Both lists between the markers are generated by
`python tools/inventory.py . --write`. The first holds every distillate of the
topic, meaning the ones its assertions ground in together with the ones whose
own `topics` names this map, so that a source registered under a topic appears
before any assertion rests on it. The second holds every assertion whose
`topics` names this map, each with its title. Everything outside the markers is
hand-written, so the lead and the open questions stay a person's text and the
generator never deletes them. A map that carries no markers yet receives its regions on
the next run. Every assertion must be reachable from at least one topic map,
and the validator raises `E-ORPHAN` on one that is not.
...
The body gives the definition in one or two sentences with a grounding anchor where the definition comes from a source. The glossary holds one document per central technical term of the content and is used as the need arises. A textual phenomenon is held as such an entry, and the assertions about it name it in `phenomena`.

The example list between the markers is generated by
`python tools/inventory.py . --write`. It holds one line per assertion that
names this entry in `phenomena`, with the grounding anchors of that assertion
indented below it, and one line saying so where no assertion declares the term.
...
A generated region stands between the two HTML comments `<!-- <name>:begin -->`
and `<!-- <name>:end -->`. The names in use are `inventory` in
[[knowledge/state]], `distillates` and `assertions` in a topic map, and
`examples` in a glossary entry. Nothing between a pair of markers is written by
hand, and nothing outside them is generated.
